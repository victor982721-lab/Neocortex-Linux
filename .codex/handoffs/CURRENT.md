...
**Estado vivo:** la verificación de integración documental confirma `HEAD ==
main == origin/main` y árbol limpio. `current` apunta a
`0.13.0-c6d3985f7a45-cp314-linux-x86_64`
(`source_sha=c6d3985f7a45fc3120bd03e9561195674f2b8ac2`), el rollback inmediato es
`0.13.0-1567fe46821b-cp314-linux-x86_64` y `.staging` está vacío. Los commits
...
activo desde `c6d3985f7a45fc3120bd03e9561195674f2b8ac2`. Mantiene las nueve rutas
...
El candidato construido desde el SHA documental más reciente se valida en un
namespace aislado con corpus y estado temporales; la promoción al namespace
canónico queda separada hasta que se cierre de forma autorizada el fence de los
procesos `agent serve` que usan una release histórica. No se modifica `current`
manualmente para sortear ese fence.

La verificación canónica del artefacto activo terminó `verified=true`; el smoke
instalado sobre 23 fixtures temporales terminó `RC1=0` y `RC2=0` en nueve rutas,
con Semantic completo usando modelos locales existentes, Archive y Text con
`cache_hits` en replay y `action_mode=dry-run`. El alcance Code `projects`
...
1. Promover la candidata al namespace canónico sólo después de cerrar de forma
autorizada los procesos `agent serve` que mantienen la release histórica; no
sortear el fence ni retirar el rollback inmediato.
