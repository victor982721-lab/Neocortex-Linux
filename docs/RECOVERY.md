...
`Neocortex --factory-reset` es una frontera distinta de backup, restore y
`databases purge`. Su único objetivo es retirar todo el estado operacional
administrado de la raíz de estado seleccionada: bases SQLite, sus sidecars,
...
destinos externos producidos por APIs standalone. El corpus y los ZIP originales,
la instalación, los modelos y los `installation-receipts` están fuera de la
operación.
...
`--state-directory` queda disponible como override para cercar el procedimiento
a un fixture. El comando no lee ni procesa el corpus, no reconstruye
materializaciones y no crea backup,
snapshot SQL, plan, digest o receipt durable adicional. No existen scopes ni
formas de `--apply`/`--yes`; la invocación explícita es el gate de la operación.
...
no se siguen ni se borran targets externos. Rutas o montajes ajenos, un writer
activo, permisos insuficientes, corrupción, cambio concurrente u objeto no
verificable producen un error con conteos parciales. Nunca se borra un sidecar
aislado para hacer pasar el resultado ni se relajan fences para continuar.

Si una parte no puede retirarse, el error conserva los conteos parciales y la
CLI termina con código distinto de cero; no se anuncia una limpieza completa
por inferencia. La operación no reconstruye el corpus, no instala releases o
modelos y no demuestra que una
corrida futura haya terminado. Para reintentar después de resolver un bloqueo,
se ejecuta una invocación nueva; no hay una receta de conciliación basada en
digest, plan o receipt para este factory reset.
...
automático mediante un runner y un verificador inyectados; la foundation por
sí sola no aporta restauración ni prueba same-filesystem.
