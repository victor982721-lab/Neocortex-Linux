...
> Esta sección describe únicamente el diseño vigente. Las versiones fechadas
> más abajo conservan hechos históricos y no son instrucciones actuales.

### Cambiado

- `code` quedó limitado a trabajar con código como contenido: detección,
  representación, persistencia, búsqueda y relaciones semánticas. La review
  interna, los experimentos, los proveedores externos, los receipts y la
  agregación de validadores ya no forman parte del runtime.
- Image conserva decodificación, OCR, clasificación industrial, semántica visual
  y búsqueda, pero ya no incluye NudeNet ni la cadena de clasificación adulta.
  El schema Image v6 reconstruye la tabla histórica sin esas columnas y permite
  un scrub explícito de las proyecciones Semantic mutables.
- Las releases Linux son product-only: no embeben Node, Pyright, Semgrep ni
  otros runtimes de QA. El lock CPython 3.14 contiene sólo la resolución de
  producción; las herramientas de desarrollo se ejecutan directamente fuera
  del runtime.
...
- No existe un quality gate agregador obligatorio. Codex ejecuta `pytest`, Ruff,
  Pyright/Mypy, Semgrep u otra herramienta individual únicamente cuando el
  cambio lo requiere y reporta el alcance comprobado.
- Las migraciones destructivas de estado requieren copia verificable, pruebas en
  copias aisladas y comprobación de integridad; no se modifican bases vivas sin
  una autorización y un preflight explícitos.
