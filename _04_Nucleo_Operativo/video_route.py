...
class VideoRoute:
    ...
    def _validate(self) -> None:
        ...
        if self.config.max_documents is not None and self.config.max_documents < 1:
            raise ValueError("video max_documents must be positive")
        if self.config.max_file_bytes is not None and self.config.max_file_bytes < 1:
            raise ValueError("video max_file_bytes must be positive")
        if self.config.max_duration_seconds <= 0:
            raise ValueError("video max_duration_seconds must be positive")
        if self.config.probe_timeout_seconds <= 0:
            raise ValueError("video probe_timeout_seconds must be positive")
        if self.config.ocr_timeout_seconds <= 0:
            raise ValueError("video ocr_timeout_seconds must be positive")
    ...
    def _run_candidates(
        ...
        for mime in sorted(VIDEO_MIME_TYPES):
            ...
            for snapshot in iterator:
                ...
                cached_status = None if cached is None else str(cached["status"])
                if cached is not None and (
                    cached_status in {"complete", "partial"}
                    or (cached_status == "error" and not self.config.retry_errors)
                ):
                ...
                metrics.processed += 1
                if metrics.processed % VIDEO_COMMIT_BATCH == 0:
                    connection.commit()
