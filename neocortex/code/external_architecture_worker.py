...
rather than ``python -m`` keeps installed application packages out of
``sys.modules`` while Grimp locates the staged canonical source tree.  Project
content is parsed statically and is never imported.
"""
...
if TYPE_CHECKING:
    ...
    from neocortex.code.contracts import target_projection as _target_projection
    from neocortex.code.contracts import target_registry as _target_registry
    ...
    _repository_root = _control_plane_root.parent.parent
    ...
    _target_registry = _load_control_plane_module(
        ...
        _control_plane_root / "contracts" / "target_registry.py",
    )
    _target_projection = _load_control_plane_module(
        "_neocortex_core_target_projection",
        _control_plane_root / "contracts" / "target_projection.py",
    )
