...
def initialize_audio_state(path: Path) -> None:
    ...
    if _existing_audio_state_is_current(path):
        return

    with audio_database(path, create=True) as connection:
        connection.execute("BEGIN IMMEDIATE")
        try:
            _initialize_locked_audio_state(connection)
...
def _existing_audio_state_is_current(path: Path) -> bool:
    if not path.is_file():
        return False
    with audio_database(path, readonly=True) as connection:
        ...
        if prior == AUDIO_SCHEMA_VERSION:
            ...
            return True
        _validate_audio_migration_source(connection, prior)
    return False


def _validate_audio_migration_source(
    ...
    elif prior not in {None, 0, AUDIO_SCHEMA_VERSION}:
        raise RuntimeError(f"unsupported audio migration start: {prior}")


def _initialize_locked_audio_state(connection: sqlite3.Connection) -> None:
    prior = read_metadata_schema_version(connection, label="audio")
    _validate_audio_migration_source(connection, prior)
    if prior == 1:
