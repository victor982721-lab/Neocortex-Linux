...
def _source_findings(
    ...
    if source is None:
        return (
            Finding(Scope.FILE, "physical_integrity_not_assessed", Certainty.UNKNOWN,
                    missing_checks=("file_integrity_inspection",)),
            Finding(Scope.PROCESSING, "published_processing_evidence_missing", Certainty.UNKNOWN,
                    missing_checks=("published_source_result",)),
        ), ()
    refs = _evidence(report, source)
    if source.status == "protected":
        return (
            Finding(Scope.FILE, "protected_not_inspected", Certainty.UNKNOWN, refs,
            ...
            Recommendation(
                Scope.PROCESSING, "inspect_with_authorized_access", refs,
    ...
    findings = (
        Finding(Scope.FILE, "physical_integrity_not_assessed", Certainty.UNKNOWN, refs,
    ...
    if source.status in {"error", "failed", "partial", "degraded"}:
        recommendations = (
            Recommendation(
                Scope.PROCESSING, "inspect_processing_evidence", refs,
...
def _observation_projection(
    ...
    if observation.kind is ObservationKind.DUPLICATE_CONTENT:
        preference_recorded = observation.selection_basis in {"explicit_user_decision", "preferred_location"}
        ...
        return (
            ...
            Finding(Scope.POLICY,
                    "keeper_preference_recorded" if preference_recorded else "preferred_keeper_location_unresolved",
                    Certainty.OBSERVED if preference_recorded else Certainty.UNKNOWN, refs, missing),
        ), (
            Recommendation(
                Scope.POLICY, "review_disposal_preconditions" if preference_recorded else
                "review_context_before_selecting_keeper", refs, missing,
    ...
    if observation.kind is ObservationKind.LOGICAL_FORMAT:
        missing = tuple(sorted(set(observation.missing_checks) | {
            "file_integrity_inspection", "opening_verification",
        }))
        return (
            Finding(Scope.FILE, observation.code, Certainty.INFERRED, refs, missing),
            Finding(Scope.POLICY, "format_identification_not_disposal_evidence",
                    Certainty.OBSERVED, refs),
        ), (
            Recommendation(
                Scope.FILE, "review_format_identification", refs, missing,
                ("explicit_metadata_or_rename_authorization", "physical_revalidation"),
            ),
        )
    return (
        Finding(Scope.DOCUMENT_CONDITION, "condition_described_in_document",
                Certainty.OBSERVED, refs, observation.missing_checks),
    ), ()
...
def build_knowledge_asset_diagnosis(
    ...
    *,
    ...
    if not isinstance(selected, tuple) or any(
        not isinstance(item, AssetDiagnosticObservation) or item.resource_id != report.resource_id
    ...
    if report.snapshot_consistency != "stable":
        ...
        return KnowledgeAssetDiagnosis(
            report.resource_id,
            tuple(Finding(scope, "snapshot_not_stable", Certainty.UNKNOWN,
                          missing_checks=("stable_owner_snapshot",)) for scope in Scope),
            (),
        )
    ...
    if report.diagnostic_gaps:
        findings.append(Finding(
            Scope.FILE, "supplemental_owner_evidence_incomplete", Certainty.UNKNOWN,
            missing_checks=report.diagnostic_gaps,
        ))
    search = facts.get(Stage.KNOWLEDGE_SEARCH)
    if search is None:
        findings.append(Finding(Scope.INDEX, "index_presence_not_verified", Certainty.UNKNOWN,
                                missing_checks=("published_index_posting",)))
    else:
        refs = _evidence(report, search)
        code = "search_eligible_not_index_verified" if search.status == "eligible" else (
            "search_projection_excluded" if search.status == "excluded" else "search_status_observed"
        )
        findings.append(Finding(Scope.INDEX, code, Certainty.OBSERVED, refs,
                                ("published_index_posting",)))
    inventory = facts.get(Stage.INVENTORY)
    inventory_refs = () if inventory is None else _evidence(report, inventory)
    path = "" if inventory is None else next(
        (item.value for item in inventory.values if item.name == "path"), ""
    )
    findings.append(Finding(
        Scope.POLICY, "dispensability_not_demonstrated", Certainty.UNKNOWN, inventory_refs,
        ("retention_and_dependency_review",),
    ))
    if PurePath(path).suffix.casefold() == ".tmp" and inventory_refs:
        findings.append(Finding(Scope.FILE, "temporary_suffix_observed", Certainty.OBSERVED,
                                inventory_refs))
        recommendations.append(Recommendation(
            Scope.POLICY, "inspect_usage_and_retention", inventory_refs,
    ...
    if not any(item.kind is ObservationKind.DOCUMENT_CONDITION for item in selected):
        findings.append(Finding(Scope.DOCUMENT_CONDITION, "document_condition_not_assessed",
                                Certainty.UNKNOWN, missing_checks=("document_claim_evidence",)))
