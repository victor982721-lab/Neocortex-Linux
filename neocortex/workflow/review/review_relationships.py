...
from neocortex.capabilities.formats.archive.logical import identify_logical_document, issue_diagnosis
from neocortex.capabilities.formats.archive.state import (
    ARCHIVE_SCHEMA_VERSION, archive_database, archive_schema_contract,
)
from neocortex.knowledge.knowledge_contracts import (
    EvidenceMethod, EvidenceRef, ResourceRef, RevisionRef, RevisionState,
)
...
from neocortex.persistence.sqlite_schema_contract import (
    read_application_schema_version, validate_sqlite_schema_contract,
)

from .review_task_contracts import (
    CanonicalJsonObject, ReviewTaskInput, ReviewTaskRecordPage, ReviewTaskSourceFence,
)
...
class ArchiveReviewSource:
    ...
    def fence(self, scope: str) -> ReviewTaskSourceFence:
        return ReviewTaskSourceFence.create(
            ...
            source_snapshot={"schema": "neocortex.archive-review-source/v1", "owner": "archive",
...
def _issue_source(row: sqlite3.Row, owner_snapshot_id: str) -> ArchiveReviewIssueSource:
    ...
    if marker_payload is not None:
        ...
        if not isinstance(markers, list) or len(markers) > 32 or any(
        ...
        if (row["document_role"] in {"logical_document", "document_component"}
                ...
                and row["identification_status"] == "identified" and identified is not None
                and identified.identified and identified.logical_kind == row["logical_kind"]):
            relation = {"container_key": row["container_key"],
                        "container_signature": row["container_signature"],
                        "logical_document_chain": row["logical_document_chain"],
                        "logical_kind": row["logical_kind"], "markers": markers,
    ...
    projection.update({"coverage_impact": coverage_impact, "recoverability": recovery,
    ...
    if row["file_key"] is not None:
        resource = ResourceRef(f"resource:archive:{row['file_key']}", "archive", "archive",
                               current_path=str(row["path"]))
        revision_payload = {key: projection[key] for key in (
            "file_key", "container_key", "container_signature", "container_size",
            "container_mtime_ns", "container_birthtime_ns", "size", "mtime_ns",
            "birthtime_ns", "processing_signature", "crc32", "text_xxh3_128",
            "document_role", "logical_document_chain",
        )}
        revision = RevisionRef(resource.resource_id, "revision:archive-review:" + _digest(revision_payload),
                               ARCHIVE_REVIEW_PRODUCER, str(row["processing_signature"]),
                               int(row["container_run_id"]), RevisionState.CURRENT)
        identifiers = (("owner", "archive"), ("archive_issue_id", str(issue_id)),
                       ("archive_owner_snapshot", owner_snapshot_id))
        if relationship_id is not None:
            identifiers += (("relationship_id", relationship_id),)
        evidence = (EvidenceRef("evidence:archive-review:" + source_digest, resource.resource_id,
                                revision.revision_id, EvidenceMethod.STRUCTURAL,
                                identifiers=identifiers, extractor=ARCHIVE_REVIEW_PRODUCER),)
    source = ReviewTaskInput(f"archive-issue:{issue_id}", "sha256", source_digest, resource, revision)
    return ArchiveReviewIssueSource(issue_id, str(row["reason_code"]), source,
                                    CanonicalJsonObject.from_mapping(projection), relationship_id, evidence)


def read_archive_review_source(
    path: Path, *, after_issue_id: int = 0, issue_ids: tuple[int, ...] | None = None,
) -> ArchiveReviewSource:
    ...
    if issue_ids is not None and (not isinstance(issue_ids, tuple) or len(issue_ids) > 100 or any(
        type(value) is not int or value <= 0 for value in issue_ids
    )):
    ...
    with archive_database(path, readonly=True) as connection:
        ...
        if issue_ids is None:
            rows = connection.execute(_SOURCE_SQL + " AND i.issue_id>? ORDER BY i.issue_id LIMIT 101",
                                      (after_issue_id,)).fetchall()
        elif issue_ids:
            rows = connection.execute(_SOURCE_SQL + " AND i.issue_id IN (" +
                                      ",".join("?" for _ in issue_ids) + ") ORDER BY i.issue_id LIMIT 101",
                                      issue_ids).fetchall()
        else:
            rows = []
...
class ReviewRelationshipResolution:
    ...
    def to_dict(self) -> dict[str, object]:
        return {"scope": "returned_review_tasks", "complete": not self.unresolved,
                ...
                "source": "published_archive_owner", "mutation_authorized": False}


def resolve_review_relationships(
    path: Path, page: ReviewTaskRecordPage, *, database: Path,
    ...
    for record in page.items:
        ...
        if record.task.task_type != ARCHIVE_REVIEW_TASK_TYPE or not record.source.input_id.startswith("archive-issue:"):
            unresolved[task_id] = "relationship_source_not_published"
            continue
        number = record.source.input_id.removeprefix("archive-issue:")
        if not number.isascii() or not number.isdecimal() or str(int(number)) != number or int(number) <= 0:
    ...
    try:
        ...
        unresolved.update((task_id, "archive_relationship_source_unavailable") for task_id in selected)
        return ReviewRelationshipResolution(unresolved=tuple(sorted(unresolved.items())))
    ...
    verified: list[str] = []
    from .review_task_repository import read_current_review_task_source_publication, read_review_task_progress
    ...
    for record in page.items:
        ...
        if current is None or current.source != record.source or source.fence(record.task.scope).source_snapshot_fingerprint != record.source_snapshot_fingerprint:
            unresolved[task_id] = "archive_relationship_source_stale"
            continue
        if record.task.scope not in source_published:
            publication = read_current_review_task_source_publication(database, source.fence(record.task.scope))
            ...
            if publication is None:
                ...
                publication_gaps[record.task.scope] = (
                    "archive_review_source_publication_missing" if progress is not None and progress.complete
        ...
        if not source_published[record.task.scope]:
            ...
            continue
        if current.relationship_id is None or not current.evidence or record.source.resource is None or record.source.revision is None:
            unresolved[task_id] = "logical_document_relationship_unproved"
            continue
        member = ReviewRelationshipMember(record.source.resource.resource_id, record.source.revision.revision_id,
    ...
    for relation_id, values in sorted(by_relation.items()):
        ...
        relationships.append(VerifiedReviewRelationship(relation_id, "archive_logical_document", members, evidence))
    return ReviewRelationshipResolution(tuple(relationships), tuple(sorted(verified)), tuple(sorted(unresolved.items())))


__all__ = ["ARCHIVE_REVIEW_TASK_TYPE", "ArchiveReviewSource", "ReviewRelationshipResolution",
           "read_archive_review_source", "resolve_review_relationships"]
