...
def build_execution_page(window: Any) -> QWidget:
    ...
    budget_row.setSpacing(14)
    profile_options = QVBoxLayout()
    profile_options.addWidget(window._field_label("Perfil de ejecución"))
    ...
    item_options = QVBoxLayout()
    item_options.addWidget(window._field_label("Límite de elementos"))
    window.max_items_spin = QSpinBox()
    window.max_items_spin.setObjectName("ExecutionMaxItems")
    window.max_items_spin.setRange(1, 100_000)
    window.max_items_spin.setValue(50)
    window.max_items_spin.setSuffix(" máx.")
    item_options.addWidget(window.max_items_spin)
    budget_row.addLayout(item_options, 1)

    deadline_options = QVBoxLayout()
    deadline_options.addWidget(window._field_label("Tiempo máximo"))
    window.deadline_spin = QSpinBox()
    window.deadline_spin.setObjectName("ExecutionDeadline")
    window.deadline_spin.setRange(1, 172_800)
    window.deadline_spin.setValue(900)
    window.deadline_spin.setSuffix(" s")
    deadline_options.addWidget(window.deadline_spin)
    budget_row.addLayout(deadline_options, 1)
    config_layout.addLayout(budget_row)
