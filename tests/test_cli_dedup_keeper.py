...
@pytest.mark.parametrize("reference_state", ["published", "unpublished", "stale"])
def test_real_cli_uses_only_current_published_code_references(
    tmp_path: Path, reference_state: str
) -> None:
    from tests.test_dedup_keeper_references import publish_code_reference_fixture

    root, state = tmp_path / "corpus", tmp_path / "state"
    consumer = _file(root, "consumer.py", b"import target\n")
    referenced = _file(root, "preferred/target (1).py", b"def value(): return 1\n")
    clean = _file(root, "clean/target.py", referenced.read_bytes())
    owner = state / "code.sqlite3"
    publish_code_reference_fixture(
        owner, (consumer, referenced, clean), published=reference_state != "unpublished"
    )
    if reference_state == "stale":
        consumer.write_bytes(b"source changed after reference publication\n")
    paths = (consumer, referenced, clean, owner)
    ...
    for _ in range(2):
        ...
        assert result.returncode == 0, result.stderr[-1000:] + result.stdout[-2000:]
        if reference_state == "published":
            assert f"KEEP {referenced}" in result.stdout
            assert "keeper_reason=verified_reference" in result.stdout
            assert (
                "KEEPER_REFERENCES status=available evaluation=available evidence_count=1"
                in result.stdout
            )
        else:
            assert f"KEEP {clean}" in result.stdout
            assert "keeper_reason=clean_name_tiebreak" in result.stdout
            assert "KEEPER_REFERENCES status=refs_unverified" in result.stdout
        preview = _cli(
            "--state-directory", str(state), "--curation-preview", "10", "--curation-json"
        )
        payload = json.loads(preview.stdout)
        item = next(item for item in payload["items"] if item["kind"] == "duplicate_group")
        factors = item["evidence"]["group_proof"]["keeper_factors"]
        if reference_state == "published":
            assert "verified_reference" in factors
            assert any("code" in factor and "head" in factor for factor in factors), factors
            assert any("revision" in factor or "version" in factor for factor in factors), factors
        else:
            assert "verified_reference" not in factors
