...
from neocortex.runtime.orchestration.inventory_projection import build_code_inventory_projection
...
def test_large_code_projection_spills_and_replays_without_live_owner(tmp_path: Path) -> None:
    calls = 0
    ...
    class Owner:
        def snapshots(self, scan_id: int):
            nonlocal calls
            calls += 1
            assert calls == 1 and scan_id == 5
            for number in range(10_000):
                yield FileSnapshot(str(tmp_path / f"file-{number}.py"), 1, number + 1, 3, 4, -1)

    projection = build_code_inventory_projection(Owner(), 5)
    assert projection.records == ()
    spool = projection._spool
    assert spool is not None
    for _ in range(2):
        values = projection.snapshots(5)
        assert sum(1 for value in values if value.path.endswith(".py")) == 10_000
    projection.close()
    assert spool.closed
    with pytest.raises(RuntimeError, match="closed"):
        next(iter(projection.snapshots(5)))


def test_code_projection_accounts_resident_memory_and_actual_spool_bytes(tmp_path: Path) -> None:
    capacity = 64 * 1024 * 1024
    coordinator = GlobalResourceCoordinator(
        ("preparation",),
        GlobalResourceLimits(
            cpu_slots=2, memory_budget_bytes=capacity,
            min_free_memory_bytes=0, min_free_commit_bytes=0,
        ),
    ...
    class Owner:
        def snapshots(self, _scan_id: int):
            for number in range(5000):
                yield FileSnapshot(str(tmp_path / f"module-{number}.py"), 1, number, 1, 1, -1)

    with resource_scope(coordinator):
        projection = build_code_inventory_projection(Owner(), 1)
        metrics = coordinator.summary().routes["preparation"]
        assert metrics.temp_bytes > 0
        assert metrics.active_execution_requests == 0
        assert metrics.peak_reserved_bytes >= 8 * 1024 * 1024
        projection.close()
        metrics = coordinator.summary().routes["preparation"]
        assert metrics.temp_bytes == 0
